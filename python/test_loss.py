...
for comparetype in ['min','max','sum','vic']:
	...
	outname='../WDataFiles/Results/testlosses_'+str(comparetype)+'.txt' if beta0==None else '../WDataFiles/Results/testlosses_beta'+str(beta0)+'_'+str(comparetype)+'.txt'
	outf=open(outname,"w")
	averageloss=0
	averagebeta=0
	for season_id in seasons:
		...
		#Prendiamo beta
		if beta0==None:
			df_beta=pd.read_csv('../WDataFiles/WRegularSeasonCompactResults_'+str(season_id)+'_'+rank_type+'_'+comparetype+'_beta.dat',sep=' ', header=None)
			beta=df_beta.iloc[0][1]
		else: beta=beta0

		testloss=cf.evaluation(beta, season_id, rank_type, epsilon, losstype, comparetype)
